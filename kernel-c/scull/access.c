...
static atomic_t scull_s_available = ATOMIC_INIT(1);
  ...
  /*----------------------------------------------------------------------\
 函数名：scull_s_open											   
 描  述：	scull_s_device 设备open函数(用到原子变量来解决并发问题)						   
 输  入：	inode 	 	:内核中设备描述
 		filp			:设备对应的文件
 输  出：	无																   
 返  回：	int			:0 操作成功								  							   
...
static int scull_s_open(struct inode *inode, struct file *filp)
{
	...
	if (atomic_dec_and_test (&scull_s_available) == 0 ) 
	{
  ...
  /*----------------------------------------------------------------------\
 函数名：scull_s_release											   
 描  述：	scull_s_release 设备release函数						   
 输  入：	inode 	 	:内核中设备描述
 		filp			:设备对应的文件
 输  出：	无																   
 返  回：	int			:0 操作成功											  							   
...
/* then, everything else is copied from the bare scull device */
  ...
  /*----------------------------------------------------------------------\
 函数名：scull_u_release											   
 描  述：	scull_u_release 设备release函数(用到自旋锁来解决并发问题)							   
 输  入：	inode 	 	:内核中设备描述
 		filp			:设备对应的文件
 输  出：	无																   
 返  回：	int			:0 操作成功								  							   
...
//WJB: SPIN_LOCK_UNLOCKED already been delete
...
static inline int scull_w_available(void)
{
  ...
  /*----------------------------------------------------------------------\
 函数名：scull_w_open											   
 描  述：	scull_w_open 设备open函数(用到 自旋锁和等待队列 来解决并发问题)							   
 输  入：	inode 	 	:内核中设备描述
 		filp			:设备对应的文件
 输  出：	无																   
 返  回：	int			:0 操作成功								  							   
...
static int scull_w_open(struct inode *inode, struct file *filp)
{
  ...
  /*----------------------------------------------------------------------\
 函数名：scull_w_release											   
 描  述：	scull_w_release 设备release函数(用到自旋锁来解决并发问题)							   
 输  入：	inode 	 	:内核中设备描述
 		filp			:设备对应的文件
 输  出：	无																   
 返  回：	int			:0 操作成功								  							   
...
#define SCULL_N_ADEVS 4
  ...
  /*----------------------------------------------------------------------\
 函数名：scull_access_setup											   
 描  述：	access 设备驱动程序初始化函数(主要完成cdev的填充)								   
 输  入：	devno 	 	:access设备设备号
 		devinfo		:设备私有机构体
 输  出：	无																   
 返  回：	无										  							   
 \----------------------------------------------------------------------*/
static void scull_access_setup (dev_t devno, struct scull_adev_info *devinfo)
{
 ...
 /*----------------------------------------------------------------------\
 函数名：scull_access_init											   
 描  述：	access 设备驱动程序初始化函数	，可初始化多个设备						   
 输  入：	firstdev 	 	:access设备号
 输  出：	无																   
 返  回：	int			:初始化的设备数量							  							   
 \----------------------------------------------------------------------*/
