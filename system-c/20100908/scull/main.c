...
int scull_read_procmem(char *buf,/*内核交给我的一个页*/ char **start, off_t offset,
                   int count, /*不超过这个范围*/ int *eof, void *data)
{
	int i, j, len = 0;
...
static void scull_create_proc(void)
{
	...
	create_proc_read_entry("scullmem", 0 /* default mode */,
			NULL /* parent dir */, scull_read_procmem,//读文件的时候调用谁来处理请求
			NULL /* client data */);
	entry = create_proc_entry("scullseq",/*文件名a*/  0 , NULL);
	if (entry)
