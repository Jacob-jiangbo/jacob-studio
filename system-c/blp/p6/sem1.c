...
static int set_semvalue(void)
{
    union semun sem_union;//设置值的时候用到了联合体
...
static int semaphore_p(void)
{
    struct sembuf sem_b;//这个是个内部的函数
    
    sem_b.sem_num = 0;
    sem_b.sem_op = -1; /* P() *///当时－1是就开始阻塞
    sem_b.sem_flg = SEM_UNDO;//会阻塞
    if (semop(sem_id, &sem_b, 1) == -1) {/////!!!用户改变信息量的直
...
static int semaphore_v(void)
{
    ...
    if (semop(sem_id, &sem_b, 1) == -1) {///！！！用户改变信息量的值
